...
import { SpikeballIcon, TrophyIcon, EditIcon } from '../components/ui/Icon'
...
function LogSBPanel({ onLogged }: { onLogged: () => void }) {
  const [open,  setOpen]  = useState(false)
  const [toast, setToast] = useState<string | null>(null)
  const [isWin, setIsWin] = useState(true)
  const refreshXP       = useStore(s => s.refreshXP)
  const refreshActivity = useStore(s => s.refreshActivity)
  const { register, handleSubmit, reset, formState: { isSubmitting } } = useForm<SBForm>({
    defaultValues: { date: today(), my_score: '', opp_score: '', partner: '', opponents: '', notes: '' },
  })

  const onSubmit = async (data: SBForm) => {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return
    const { error } = await supabase.from('spikeball_games').insert({
      user_id:   user.id,
      date:      data.date,
      win:       isWin,
...
function EditSBModal({ game, onClose, onSaved }: { game: SpikeballGame; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    setSaving(true)
    const { error } = await supabase.from('spikeball_games').update({
  ...
  async function del() {
    const { error } = await supabase.from('spikeball_games').delete().eq('id', game.id)
    if (!error) { onSaved(); onClose() }
  }
...
export function Spikeball() {
  ...
  const [games,   setGames]   = useState<SpikeballGame[]>([])
  const [editing, setEditing] = useState<SpikeballGame | null>(null)

  async function load() {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return
    const { data } = await supabase.from('spikeball_games').select('*').eq('user_id', user.id).order('date', { ascending: false })
    setGames(data ?? [])
  }
  useEffect(() => { load() }, [])
  ...
  return (
    <>
      ...
      <PageWrapper>
        ...
        {games.length === 0 && (
          <EmptyState
            icon={<SpikeballIcon size={56} color="var(--text-muted)" />}
            title="No games logged yet"
            sub="Log your first spikeball game to start tracking wins and streaks."
          />
        )}

        {editing && <EditSBModal game={editing} onClose={() => setEditing(null)} onSaved={load} />}
      </PageWrapper>
    </>
  )
}
